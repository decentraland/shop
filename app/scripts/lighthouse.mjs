...
import { METRICS, entryChunkOf, medians, restoredSession, summarize } from './lighthouse-lib.mjs'
...
async function collect() {
  ...
  for (const file of files.sort()) {
    ...
    // Belt and braces for reports written before the per-run check existed: a file labelled signed-in that
    // never loaded the notifications chunk is a guest run, and folding it into a signed-in row would
    // understate exactly what that row exists to measure.
    if (meta.session === 'seeded' && !restoredSession(lhr)) {
      console.warn(`skipping ${file}: labelled signed-in but no session restored`)
      continue
    }
