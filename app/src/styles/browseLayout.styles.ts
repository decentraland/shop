...
// Shared browse layout used by Assets, MyAssets, Collection and Creator: a 265px filter sidebar + a
// fluid main column (toolbar + grid). On desktop the sidebar is sticky (pinned below the navbar +
// sub-nav) and scrolls internally when its sections overflow the viewport, so it accompanies the
// grid's scroll. Below the `lg` breakpoint the sidebar becomes an off-canvas bottom-sheet Filters
// drawer opened by the toolbar's Filters pill; the grid then takes the full width.
//
// Import as `import * as BL from '~/styles/browseLayout.styles'`.
