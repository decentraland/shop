  ...
  return (
    <S.Footer>
      <S.Main>
        ...
        <S.MobileMenu>
          <S.MenuLabel>{t('footer.menu')}</S.MenuLabel>
          {[
      ...
      <S.Bottom>
        <S.BottomLeft>
          ...
          <S.Legal>
            {legalLinks.map(l => (
              <S.LegalLink key={l.label} href={l.url} target="_blank" rel="noopener noreferrer">
                {t(l.label)}
              </S.LegalLink>
            ))}
          </S.Legal>
        ...
        <S.Copy>&copy; {new Date().getFullYear()} Decentraland</S.Copy>
      </S.Bottom>
    </S.Footer>
  )
}
